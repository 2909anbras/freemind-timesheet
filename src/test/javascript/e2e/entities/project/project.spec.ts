...
describe('Project e2e test', () => {
  ...
  it('should load Projects', async () => {
    ...
    expect(await projectComponentsPage.getTitle()).to.eq('freemindTimesheetApp.project.home.title');
  ...
  it('should load create Project page', async () => {
    ...
    expect(await projectUpdatePage.getPageTitle()).to.eq('freemindTimesheetApp.project.home.createOrEditLabel');
  ...
  it('should create and save Projects', async () => {
    ...
    if (await selectedEnable.isSelected()) {
      await projectUpdatePage.getEnableInput().click();
      expect(await projectUpdatePage.getEnableInput().isSelected(), 'Expected enable not to be selected').to.be.false;
    } else {
      await projectUpdatePage.getEnableInput().click();
      expect(await projectUpdatePage.getEnableInput().isSelected(), 'Expected enable to be selected').to.be.true;
    }
  ...
  it('should delete last Project', async () => {
    ...
    expect(await projectDeleteDialog.getDialogTitle()).to.eq('freemindTimesheetApp.project.delete.question');
