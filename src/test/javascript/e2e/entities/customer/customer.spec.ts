...
describe('Customer e2e test', () => {
  ...
  it('should load Customers', async () => {
    ...
    expect(await customerComponentsPage.getTitle()).to.eq('freemindTimesheetApp.customer.home.title');
  ...
  it('should load create Customer page', async () => {
    ...
    expect(await customerUpdatePage.getPageTitle()).to.eq('freemindTimesheetApp.customer.home.createOrEditLabel');
  ...
  it('should create and save Customers', async () => {
    ...
    await promise.all([customerUpdatePage.setNameInput('name'), customerUpdatePage.companySelectLastOption()]);

    expect(await customerUpdatePage.getNameInput()).to.eq('name', 'Expected Name value to be equals to name');
    const selectedEnable = customerUpdatePage.getEnableInput();
    if (await selectedEnable.isSelected()) {
      await customerUpdatePage.getEnableInput().click();
      expect(await customerUpdatePage.getEnableInput().isSelected(), 'Expected enable not to be selected').to.be.false;
    } else {
      await customerUpdatePage.getEnableInput().click();
      expect(await customerUpdatePage.getEnableInput().isSelected(), 'Expected enable to be selected').to.be.true;
    }
  ...
  it('should delete last Customer', async () => {
    ...
    expect(await customerDeleteDialog.getDialogTitle()).to.eq('freemindTimesheetApp.customer.delete.question');
